...
class SettingsController extends Controller
{
    ...
    public function setting()
    {
        ...
        $data['title']  = 'Settings';
        $data['user']   = $user;
        return view('admin.super_admin.settings', compact('data'));
    }
    public function profileUpdate($id, Request $request)
    {
        ...
        try {
            ...
            if ($request->image) {
                ...
                $file_path = 'uploads/admin_profile';
    ...
    public function passUpdate($id, Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            Toastr::error(trans($firstError), 'Error', ["positionClass" => "toast-top-right"]);
            return redirect()->back();
        ...
        try {
            ...
            Toastr::error(trans('Admin password not updated!'), 'Error', ["positionClass" => "toast-top-right"]);
            return redirect()->back();
        }
        DB::commit();
        // return response()->json(['status' => 1, 'message' => 'User Password Updated Successfully!'], 200);
        Toastr::success(trans('Admin Password Updated Successfully!'), 'Success', ["positionClass" => "toast-top-right"]);
        return redirect()->back();
    }
    ...
    {
        if (is_null($this->user) || !$this->user->can('admin.settings')) {
    ...
    {
        if (is_null($this->user) || !$this->user->can('admin.settings.general_store')) {
            abort(403, 'Sorry !! You are Unauthorized.');
        }
        ...
        try {
            ...
            $setting->apple_client_id          = $request->apple_client_id;
            $setting->apple_client_secret      = $request->apple_client_secret;
            ...
            $setting->instagram_url             = $request->instagram_url;
            $setting->tawk_src                  = $request->tawk_src;
            $setting->tawk_chat_url             = $request->tawk_chat_url;
            $setting->mailchimp_api_key         = $request->mailchimp_api_key;
            $setting->mailchimp_list_id         = $request->mailchimp_list_id;
            $setting->flutterwave_public_key    = $request->flutterwave_public_key;
            $setting->flutterwave_secret_key    = $request->flutterwave_secret_key;
            $setting->flutterwave_encription_key= $request->flutterwave_encription_key;
            ...
            $setting->recaptcha_enable          = $request->recaptcha_enable ? 1 : 0;
            ...
            $this->changeEnv([
                ...
                'ENCRYPTION_KEY'                => $request->flutterwave_encription_key,
                'PAYPAL_MODE'                   => $request->paypal_mode,
                'PAYPAL_SANDBOX_CLIENT_ID'      => $request->paypal_client_id,
                'PAYPAL_SANDBOX_CLIENT_SECRET'  => $request->paypal_client_secret,
            ...
            dd($e->getMessage());
            DB::rollback();
            Toastr::error(trans('Unable to update settings'), 'Error', ["positionClass" => "toast-top-right"]);
            return redirect()->back();
        }
