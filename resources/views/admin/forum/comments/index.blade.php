                ...
                <div class="card" >
                    <div class="card-header" >
                        <div class="d-flex align-items-center justify-content-between">
                            <div>
                                <h3 class="card-title" >All Forum Comments</h3>
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody>
                                @foreach ($data['rows'] as $key => $row)
                                    <tr>
                                        ...
                                        <td><a class="{{ $row->status == 1 ? 'text-green' : '' }}" href="{{route('frontend.forum.details',['slug'=>$row->getForum->slug])}}">{{$row->getForum->title}}</a></td>
                                        ...
                                        <td class="text-center">
                                            @if($row->status ==1)
                                                <a href="{{ route('admin.forum.comment.updateForumCommentStatus',['id'=>$row->id]) }}" class="btn btn-sm btn-warning">Unpublish</a>
                                            @else
                                                <a href="{{ route('admin.forum.comment.updateForumCommentStatus',['id'=>$row->id]) }}" class="btn btn-sm btn-success">Publish</a>
                                            @endif
                                        </td>
                                        <td class="text-center">
                                        ...
                                        <td class="text-center">
                                            <a href="{{ route('admin.forum.comment.view',['id' => $row->id]) }}"
                                                class="btn btn-sm" style="background: #9C9C9C" title="View">
                                                <i class="far fa-eye" style="color: #ffffff;"></i>
                                            </a>
                                            <a href="{{ route('admin.forum.comment.delete', $row->id ) }}" title="Delete"
