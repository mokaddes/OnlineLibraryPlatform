...
--------------------------*/
            ...
            $('#fb5-book').turn({
                ...
                when: {
                    ...
                    turned: function (e, page) {
                        // pageTurnedWithinInterval = true;
                        // $("#fb5-book").turn("disable", false)
                        ...
                        var totalLength = Book_v5.getLength();
                        var pageStayTime = reading_time / totalLength;
                        /* if (DISPLAY === "double" && page1 !== page) {
                             pageStayTime = reading_time / (totalLength / 2);
                         }*/

                        if (!readingCompleteStatus) {

                            var display = document.querySelector('#safeTimerDisplay');
                            var pageTime = document.querySelector('#pageTimerDisplay');

                            if (page > currentPage) {
                                savePageStayTime(timer, currentPage)
                            }
                            $.ajax({
                                type: 'GET',
                                url: progressRoute,
                                data: {
                                    type: 'get',
                                    page: page1,
                                    page_total_time: pageStayTime,
                                    total_page: totalLength,
                                    previous_time : timer
                                },
                                success: function (response) {
                                    ...
                                    if (response.status) {
                                        ...
                                        pageTime.textContent = timeToDisplay(pageStayTime);
                                        pageStayTime = response.stay_time;
                                        display.textContent = "00:00:00";
                                        startTimer(pageStayTime, display, page1, page, totalLength);
	...
	/*/
        ...
        nextPage: function () {
            ...
            // if (!pageTurnedWithinInterval) {
            //     return false;
            // }

            var currentPages = Book_v5.getCurrentPage();
            $('#fb5-book').turn('next');
            /*
      var current=$('#fb5-book').turn('page');
      if( current%2==0){
